...
$code_reponse = 406;
    ...
    {	// récupération de la liste des utilisateurs à l'aide de la méthode getTousLesUtilisateurs de la classe DAO
        ...
        if ($nbReponses == 0) {
            $msg = "Aucun autorisation accordée à " .$pseudo.".";
            $code_reponse = 200;
        }
        else {
...
{	if ( $dao->getNiveauConnexion($pseudo, $mdpSha1) == 0 )
    ...
    {	// récupération de la liste des utilisateurs à l'aide de la méthode getTousLesUtilisateurs de la classe DAO
        ...
        $lesUtilisateurs = $dao->getLesUtilisateursAutorisant($Utilisateur->getId()); 
        
        // mémorisation du nombre d'utilisateurs
        $nbReponses = sizeof($lesUtilisateurs);
        
        if ($nbReponses == 0) {
            $msg = "Aucune autorisation accordée par ". $Utilisateur->getPseudo();
        }
        else {
            $msg = $nbReponses . " autorisation(s) accordée(s) par ". $Utilisateur->getPseudo();
        }
    }
}
// ferme la connexion à MySQL
